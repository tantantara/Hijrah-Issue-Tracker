...
<head>
    ...
    <style>
        ...
        .btn-add { background: #007bff; color: white; text-decoration: none; padding: 10px 20px; border-radius: 5px; font-weight: bold; font-size: 14px; }
        ...
        /* ==========================================
           ...
           ========================================== */
        @media print {
            .site-sidebar, .btn-print, .btn-add, .back-btn, .header-box, .btn-edit-side { display: none !important; }
            ...
            @page {
                size: landscape;
                margin: 0.5cm 1cm; /* Kecilkan margin atas untuk sorok title browser */
            }
        }
    </style>
</head>
...
<div class="site-sidebar">
    <a href="engineer_dash.php" class="back-btn">← Back to Dashboard</a>
    ...
    while($s = mysqli_fetch_assoc($sites)) {
        ...
        echo "<a href='engineer_select_site.php?folder_id=$folder_id&site_id={$s['id']}' class='site-link $active'>📍 {$s['site_name']}</a>";
    }
    ?>
</div>
    ...
    <?php if($site_id > 0): ?>
        <div class="header-box">
            ...
            <div style="display: flex; gap: 10px;">
                <a href="engineer_add_issue.php?folder_id=<?php echo $folder_id; ?>&site_id=<?php echo $site_id; ?>" class="btn-add">Add New Issue</a>
                        ...
                        while($row = mysqli_fetch_assoc($issues)) {
                            ...
                            echo "<tr>
                                    ...
                                    <td style='position: relative;'>
                                        ".($row['close_time_date'] ? date('d/m/Y H:i', strtotime($row['close_time_date'])) : '-')."
                                        <a href='edit_issue.php?id={$row['id']}' class='btn-edit-side'>EDIT</a>
                                    </td>
                                  </tr>";
                        }
                    } else {
